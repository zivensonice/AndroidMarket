...
import com.ziven.androidmarket.BaseActivity;
...
public class PagerTab extends ViewGroup {
	...
	private void addTextTab(final int position, String title) {
		...
		tab.setSingleLine();
		// 设置字体为PX单位,dp->px->设置到字体.setTextSize默认设置字体为sp
	...
	private class PageListener implements OnPageChangeListener {
		...
		public void onPageScrollStateChanged(int state) {
			/*
			 * 总共有三种状态touch fling和idle
			 * 不管怎么样肯定会经历idle状态,所以把手指点了控件,但是又控制控件滑动这种特性行为 写在idle里面
			 */
			if (state == ViewPager.SCROLL_STATE_IDLE) {
				mSelectedPosition = -1;
			}
			/* 转发给ViewPager 使页面状态做相应的改变 ,实现的是主页面和指针控件联动变化 */
			if (mDelegatePageListener != null) {
